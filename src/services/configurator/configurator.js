...
function setupEventListeners() {
    ...
    document.getElementById('loadConfigs').addEventListener('click', loadSavedConfigurations);
    document.getElementById('createOrder').addEventListener('click', createOrder);
}

async function loadProducts() {
    try {
        const response = await sendRequest('http://localhost:3000/market/product/get', {
            method: 'POST',
            body: JSON.stringify({
    ...
    container.innerHTML = rooms.map(room => `
        <div class="room">
            <div class="room-header">
                ...
                <button class="btn btn-sm btn-danger" onclick="removeRoom(${room.id})">×</button>
            </div>
                    ...
                    return `
                        <div class="product-item" data-product-id="${product.id}">
                            ...
                            <button class="btn btn-sm btn-danger ms-2" 
                                    onclick="removeProductFromRoom(${room.id}, ${product.id})">×</button>
                        </div>
                    `;
                }).join('')}
                ${room.products.length === 0 ? '<p class="text-muted">Перетащите продукты сюда</p>' : ''}
            </div>
        </div>
    `).join('');
...
function updateStats() {
    ...
    document.getElementById('productsCount').textContent = uniqueProductIds.length;
}

async function saveConfiguration() {
    try {
        const name = document.getElementById('configName').value || 'Конфигурация ' + new Date().toLocaleDateString();
...
async function loadSavedConfigurations() {
    try {
        const configs = await sendRequest('http://localhost:3000/market/configurator/list', {
            method: 'GET'
        });
        ...
        if (configs.length === 0) {
            ...
            list.innerHTML = configs.map(config => `
                <div class="mb-2 p-2 border rounded">
                    ...
                    <small>${formatCurrency(config.totalPrice)} | ${new Date(config.createdAt).toLocaleDateString()}</small><br>
                    <button class="btn btn-sm btn-primary mt-1" onclick="loadConfiguration(${config.id})">Загрузить</button>
                    <button class="btn btn-sm btn-danger mt-1" onclick="deleteConfiguration(${config.id})">Удалить</button>
                </div>
            `).join('');
        }
...
window.deleteConfiguration = async function(configId) {
    try {
        await sendRequest(`http://localhost:3000/market/configurator/delete?id=${configId}`, {
            method: 'DELETE'
        });
...
async function createOrder() {
    ...
    showSuccessNotification('Функция создания заказа из конфигурации будет реализована');
}
